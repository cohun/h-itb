...
export async function getStaticProps(context) {
  ...
  const res = await client.getEntries({
    ...
    "fields.productGroupId[all]": "A.4232.",
    locale: "hu-HU",
  });

  // Here A.411. must be changed according to the actual productGroup
  const resFiltered = res.items.filter(
    (item) => item.fields.productGroupId === "A.4232."
  );
...
function G100LiftingHooks({ type }) {
  return (
    <div>
      <Head>
        <title>G100 horgok</title>
        <meta name="description" content="G100 horgok" />
      </Head>
          ...
          <ul>
            ...
            <li>
              <Link
                ...
                passHref
              >
                <div className="has-text-grey px-3">Horgok</div>
              </Link>
            </li>
            <li>
              <Link
...
export default G100LiftingHooks;
