...
export async function getStaticProps(context) {
  ...
  const res = await client.getEntries({
    content_type: 'productTable', 'fields.productGroupId[all]': 'C.22.',
    locale: 'hu-HU',
  });
...
function Stackers({ type }) {
  return (
    <div>
      <Head>
        <title>Gutman targonca adapter</title>
        <meta name="description" content="Gutman targonca adapter" />
      </Head>
          ...
          <ul>
            ...
            <li>
              <Link href="/MaterialHandling/Attachments" passHref>
                <div className="has-text-grey px-3">Adapterek</div>
              </Link>
            </li>
            <li>
              <Link
                ...
                passHref
              >
                <div className="is-active px-3">Targoncadapterek</div>
              </Link>
            </li>
          </ul>
        </nav>
